    ...
    public static HashMap<String, String> parse(File filePath) {
        ...
        EGRIP_UL_DATA.UL.each {
            ...
            if (it.UL_FINISH.REGORG.@ID != "") {
                idregorgend = (Spregorg) EgrulDAO.getFromDB(Spregorg.getName(), Util.convertToBInt(it.UL_FINISH.REGORG.@ID))
                if (idregorgend == null) {
                    idregorgend = new Spregorg(
                            idspro: Util.convertToBInt(it.UL_FINISH.REGORG.@ID),
                            name: it.UL_FINISH.REGORG.@NAME
                    )
                    EgrulDAO.saveOrUpdate(idregorgend)
                }
            }
            Spregorg idregorgstart = null
            if (it.UL_START.REGORG.@ID != "") {
                idregorgstart = (Spregorg) EgrulDAO.getFromDB(Spregorg.getName(), Util.convertToBInt(it.UL_START.REGORG.@ID))
                if (idregorgstart == null) {
                    ...
                    EgrulDAO.saveOrUpdate(idregorgstart)
                }
            }
            Spregorg idregorg = (Spregorg) EgrulDAO.getFromDB(Spregorg.getName(), Util.convertToBInt(it.REGORG.@ID))
            if (idregorg == null) {
                idregorg =
                        new Spregorg(
                ...
                EgrulDAO.saveOrUpdate(idregorg)
            }
            Spvidreg spvidregend = null
            if (it.UL_FINISH.VIDREG.@ID != "") {
                spvidregend = (Spvidreg) EgrulDAO.getFromDB(Spvidreg.getName(), Util.convertToInt(it.UL_FINISH.VIDREG.@ID))
                if (spvidregend == null) {
                    spvidregend =
                            new Spvidreg(
                    ...
                    EgrulDAO.saveOrUpdate(spvidregend)
                }
            }
            Spvidreg spvidregstart = null
            if (it.UL_START.VIDREG.@ID != "") {
                spvidregstart = (Spvidreg) EgrulDAO.getFromDB(Spvidreg.getName(), Util.convertToInt(it.UL_START.VIDREG.@ID))
                if (spvidregstart == null) {
                    ...
                    EgrulDAO.saveOrUpdate(spvidregstart)
                }
            }
            ...
            if (it.OKVED.@KOD_OKVED != "") {
                ...
                it.OKVED.each {
                    Okved okved = (Okved) EgrulDAO.getNamedQuery(Okved.FIND_BY_CODEOKVED, "codeOkved", it.@KOD_OKVED as String)
                    if (okved == null) {
                        okved = new Okved(
            ...
            if (it.UL_NAME.@DTSTART != "") {
                it.UL_NAME.each {
                    Splang splang = null
                    if (it.NAMEI.LANG.@ID != "") {
                        splang = (Splang) EgrulDAO.getFromDB(Splang.getName(), Util.convertToInt(it.NAMEI.LANG.@ID))
                        if (splang == null) {
                            splang =
                                    new Splang(
...
/*                    String temp = (it.ADDRESS.REGION.@NAME.text()) + " " + (it.ADDRESS.RAION.@NAME.text())+ " " +
            ...
            if (it.UCHR.RUL.@DTSTART != "") {
                ...
                it.UCHR.RUL.each {
                    ...
                    if (it.UCHR.RUL.REGORG.@ID != "") {
                        idregorgrul = (Spregorg) EgrulDAO.getFromDB(Spregorg.getName(), Util.convertToBInt(it.REGORG.@ID))
                        if (idregorgrul == null) {
                            idregorgrul =
                                    new Spregorg(
                                            idspro: Util.convertToBInt(it.REGORG.@ID),
                            ...
                            EgrulDAO.saveOrUpdate(idregorgrul)
                        }
                    }
                    it.ADDRESS.each {
                        if (it.REGION.@ID != "") {
            ...
            if (it.PREDSH.@NAMEP != "") {
                ...
                it.PREDSH.each {
                    ...
                    if (it.PREDSH.REGORG.@ID != "") {
                        idregorgrpred = (Spregorg) EgrulDAO.getFromDB(Spregorg.getName(), Util.convertToBInt(it.REGORG.@ID))
                        if (idregorgrpred == null) {
                            ...
                            EgrulDAO.saveOrUpdate(idregorgrpred)
                        }
                    }
                    it.ADDRESS.each {
                        if (it.REGION.@ID != "") {
            ...
            if (it.PREEM.@NAMEP != "") {
                ...
                it.PREEM.each {
                    ...
                    if (it.PREDSH.REGORG.@ID != "") {
                        idregorgrpreem = (Spregorg) EgrulDAO.getFromDB(Spregorg.getName(), Util.convertToBInt(it.REGORG.@ID))
                        if (idregorgrpreem == null) {
                            ...
                            EgrulDAO.saveOrUpdate(idregorgrpreem)
                        }
                    }
                    it.ADDRESS.each {
                        if (it.REGION.@ID != "") {
            ...
            if (it.LICENZ.@NUMLIC != "") {
                ...
                Licenz licenz;
                it.LICENZ.each {
                    Splicorg splicorg = (Splicorg) EgrulDAO.getFromDB(Splicorg.getName(), it.LICORG.@ID as String)
                    if (splicorg == null) {
                        splicorg = new Splicorg(
                                id: it.LICORG.@ID as String,
                                name: it.LICORG.@NAME
                        )
                        EgrulDAO.saveOrUpdate(splicorg)
                    }

                    Spsostlic spsostlic = (Spsostlic) EgrulDAO.getFromDB(Spsostlic.getName(), Util.convertToInt(it.SOSTLIC.@ID))
                    if (spsostlic == null) {
                        spsostlic = new Spsostlic(
                                idsostlic: Util.convertToInt(it.SOSTLIC.@ID),
                                name: it.SOSTLIC.@NAME
                        )
                        EgrulDAO.saveOrUpdate(spsostlic)
                    }

                    Spvidlic spvidlic = null
                    if (it.VIDLIC.@ID != "") {
                        spvidlic = (Spvidlic) EgrulDAO.getFromDB(Spvidlic.getName(), Util.convertToBInt(it.VIDLIC.@ID))
                        if (spvidlic == null) {
                            ...
                            EgrulDAO.saveOrUpdate(spvidlic)
                        }
                    }
                    licenz = new Licenz(
                            idul: ul,
            ...
            it.REGEGRUL.each {
                Spregorg spregorg = (Spregorg) EgrulDAO.getFromDB(Spregorg.getName(), Util.convertToBInt(it.REGORG.@ID))
                if (spregorg == null) {
                    spregorg = new Spregorg(
                            idspro: Util.convertToBInt(it.REGORG.@ID),
                            name: it.REGORG.@NAME)
                    EgrulDAO.saveOrUpdate(spregorg)
                }

                Spvidreg spvidreg = (Spvidreg) EgrulDAO.getFromDB(Spvidreg.getName(), Util.convertToInt(it.VIDREG.@ID))
                if (spvidreg == null) {
                    spvidreg = new Spvidreg(
                            idvidreg: Util.convertToInt(it.VIDREG.@ID),
                            name: it.VIDREG.@NAME
                    )
                    EgrulDAO.saveOrUpdate(spvidreg)
                }
            ...
            if (it.MNS.@DTSTART != "") {
                it.MNS.each {
                    Spmns spmns = (Spmns) EgrulDAO.getFromDBbyNaturalId(Spmns.getName(), it.ORGAN_MNS.@KOD as String)
                    if (spmns == null) {
                        spmns = new Spmns(
                                kod: it.ORGAN_MNS.@KOD,
                                name: it.ORGAN_MNS.@NAME
                        )
                        EgrulDAO.saveOrUpdate(spmns)
                    }
            ...
            if (it.PF.@REGN_PF != "") {
                it.PF.each {
                    Sppf sppf = (Sppf) EgrulDAO.getFromDBbyNaturalId(Sppf.getName(), it.ORGAN_PF.@KOD as String)
                    if (sppf == null) {
                        sppf = new Sppf(
                                kod: Util.convertToInt(it.ORGAN_PF.@KOD),
                                name: it.ORGAN_PF.@NAME
                        )
                        EgrulDAO.saveOrUpdate(sppf)
                    }
            ...
            if (it.FSS.@REGN_FSS != "") {
                it.FSS.each {
                    Spfss spfss = (Spfss) EgrulDAO.getFromDBbyNaturalId(Spfss.getName(), it.ORGAN_FSS.@KOD as String)
                    if (spfss == null) {
                        spfss = new Spfss(
                                kod: it.ORGAN_FSS.@KOD,
                                name: it.ORGAN_FSS.@NAME
                        )
                        EgrulDAO.saveOrUpdate(spfss)
                    }
/*                    Ulfss ipfss = (Ulfss) PersistEgrip.getFromDB(Ulfss.getName(), ul.getIdip() as Integer)
            ...
            if (it.FOMS.@REGN_FOMS != "") {
                it.FOMS.each {
                    Spfoms spfoms = (Spfoms) EgrulDAO.getFromDBbyNaturalId(Spfoms.getName(), it.ORGAN_FOMS.@KOD as String)
                    if (spfoms == null) {
                        spfoms = new Spfoms(
